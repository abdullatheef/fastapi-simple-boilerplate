...
from src.app.api.dependencies import get_current_sample_user
...
def test_login_user(db: Session, client: TestClient) -> None:
    password = '123'
    user = generators.create_user(db, password)
    print("ggg>>>>>>>>")
    response = client.post(f"/api/v1/my-login", json={"username": user.username, "password": password})
    print(">>>>>>>>")
    print(response.text, ">>>>>>>>")
