...
async def authenticate_sample_user(username_or_email: str, password: str, db: AsyncSession) -> dict[str, Any] | Literal[False]:
    if "@" in username_or_email:
        db_user: dict | None = await crud_sample_user.get(db=db, email=username_or_email, is_deleted=False)
    else:
        db_user = await crud_sample_user.get(db=db, username=username_or_email, is_deleted=False)
